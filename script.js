...
// === SUBMIT TO FORMSPREE FUNCTIONALITY ===
...
document.getElementById('submit-order')?.addEventListener('click', () => {
  if (!generatedZipBlob) {
    alert(translations[currentLang].pleaseDownloadFirst || "Please download the ZIP file first!");
    return;
  }

  const email = prompt(translations[currentLang].enterEmail || "Please enter your email:", "");
  if (!email || !email.includes('@')) return;

  const orderNo = prompt(translations[currentLang].enterOrderNo || "Please enter your Order Number:", "");
  if (!orderNo) return;

  document.getElementById('customer-email').value = email;
  document.getElementById('order-number').value = orderNo;

  const file = new File([generatedZipBlob], "custom-knives.zip", { type: "application/zip" });
  const dt = new DataTransfer();
  dt.items.add(file);
  document.getElementById('zip-file-input').files = dt.files;

  alert(translations[currentLang].submitting || "Submitting your design... Thank you!");
